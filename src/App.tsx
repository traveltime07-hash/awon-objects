...
const fakeApi = {
  async addProperty(p: Partial<Property>) { await sleep(150); return { ok: true, id: crypto.randomUUID() } as const; },
  async deleteProperty(id: string) { await sleep(120); return { ok: true } as const; },
  async saveProperty(partial: any) { await sleep(120); return { ok: true } as const; },
  async saveInvoice(payload: any) { await sleep(120); return { ok: true } as const; },
  async addRoom(room: any) { await sleep(100); return { ok: true, id: crypto.randomUUID() } as const; },
...
function PropertyScreen({
  ...
  const addMember = async () => {
    ...
    setBusy(false); onPersist(updated); toast("Dodano członka zespołu");
  };
  ...
  const startEdit = (m: Member) => {
    ...
    setEditDraft({
      ...
      email: m.email,
      password: "",
      roleSel: isStd ? m.role : "Custom",
      customRole: isStd ? "" : m.role,
    });
  };
  ...
  const saveEdit = async (id: string) => {
    ...
    const updated: Property = {
      ...property,
      members: property.members.map((m) => (m.id === id ? { ...m, name: editDraft.name, email: editDraft.email, role, ...(editDraft.password ? { password: editDraft.password } : {}) } : m)),
    };
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <Card title="Dane do faktury" footer={<Button variant="primary" disabled={busy} onClick={saveInvoice}>Zapisz dane do faktury</Button>}>
              <div className="flex justify-end">
                <Button variant="outline" onClick={copyBasicsToInvoice}>Skopiuj z danych podstawowych</Button>
              </div>
              <FormGrid>
...
function PermissionsModal({
  ...
  const setRoomScope = (mode: "all" | "selected") => {
    setState((s) => ({
      ...s,
      calendar: { ...s.calendar, roomScope: mode, roomIds: mode === "all" ? rooms.map((r) => r.id) : s.calendar.roomIds },
    }));
  };
...
function Button({
  ...
  const className = isGhost
    ? `${base} text-indigo-700 hover:bg-indigo-50 border border-transparent`
    : isOutline
...
function seedProperty(name: string, city: string): Property {
  const rooms: Room[] = [{ id: crypto.randomUUID(), name: "Apartament 1" }, { id: crypto.randomUUID(), name: "Apartament 2" }];
